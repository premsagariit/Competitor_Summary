...
from and how, for auditability:
  ...
  - "derived" specs (`DERIVED_METRIC_SPECS`): a purely-documentary registry
    ...
    phase2_extraction.compute_derived_metrics(). The formula bodies
...
DERIVED_METRIC_SPECS = [
    ...
    {"key": "facility_rent_per_office_month", "slide": 22, "metric1": "Facility rental per office per month", "metric2": None,
     "kind": "money", "source": "derived", "companies": "all",
     "formula": "compute_derived_metrics: rent_cur * 100 / 9 / offices_cur",
     "inputs": ["rent_expense (NL-7, llm, cumulative YTD Q3)", "offices_count (NL-41, llm, current period only)"],
     "notes": "The '/9' divides the 9-month YTD Q3 rent figure into a monthly run-rate - "
              "QUARTER-SPECIFIC: this constant must become '/months-elapsed-in-FY' if this "
              "is ever reused for a Q1/Q2/Q4 run. Current-period only (NL-41 has no prior column)."},
    ...
    {"key": "hist_pbt_27", "slide": 27, "metric1": "PBT", "metric2": None,
     "kind": "money", "source": "derived", "companies": "all",
     "formula": "compute_derived_metrics: passthrough of income['pbt'], or income['pbt_for_quarter'] override",
     "inputs": ["pbt (NL-2, deterministic income statement)", "pbt_for_quarter (NL-2 text-fallback only)"],
     "notes": "Normally the same cumulative figure as Slide 18's PBT (GT-verified for every "
              "company). Exception: a company whose NL-2 only has a gridline-less text "
              "fallback (currently Narayana Health) has a GT figure here that matches the "
              "'For the Quarter' sub-column instead - see extract_income_statement's "
              "'PBT (For the Quarter)' key."},
    ...
    {"key": "investment_yield_32", "slide": 32, "metric1": "Investment Yield", "metric2": None,
     "kind": "percent", "source": "derived", "companies": "all",
     "formula": "phase2_extraction.extract_investment_yield: NL-31's own TOTAL row Gross Yield sub-column, read directly",
     "inputs": ["NL-31 TOTAL row (deterministic, pdf_extract.py)"],
...
def audit(ws_path="Data_Engine_UI.xlsx"):
    ...
    wb = openpyxl.load_workbook(ws_path)
    ws = wb["Data Engine"]
